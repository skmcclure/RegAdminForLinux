dnl @synopsis AX_JM_PREFIX_CONFIG_H [(OUTPUT-HEADER [,PREFIX [,ORIG-HEADER]])]
dnl
...
dnl to place the AX_JM_PREFIX_CONFIG_H call before the AC_OUTPUT
dnl invokation.
dnl
dnl Example:
dnl
...
dnl   AX_JM_PREFIX_CONFIG_H(mylib/_config.h) # prep mylib/_config.h from it..
...
dnl
dnl if the argument to AX_JM_PREFIX_CONFIG_H would have been omitted then
...
AC_DEFUN([AX_JM_PREFIX_CONFIG_H],[ dnl AC_REQUIRE([AC_CONFIG_HEADER])
  AC_CONFIG_COMMANDS([ifelse($1,,$PACKAGE-config.h,$1)],[dnl
    ...
    if test "x$_INP" = "x"; then
       for ac_file in "" $ac_prefix_conf_CONFIG_HEADERS; do
    ...
    if test -z "$_PKG" ; then
      ...
      if test -f $_INP ; then
        ...
        echo 's/'$_UPP'_'$_UPP'/'$_UPP'/g' >> _script
        echo 's/_'$_LOW'_'$_LOW'/'$_LOW'/g' >> _script
        ...
        if test -f $_OUT && cmp -s $_OUT $tmp/pconfig.h 2>/dev/null; then
    ...
    PACKAGE="$PACKAGE"
    ac_prefix_conf_CONFIG_HEADERS="$ac_config_headers"
  ])
])
