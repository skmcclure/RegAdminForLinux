dnl @synopsis AX_JM_BOOST_DATE_TIME
dnl
...
AC_DEFUN([AX_JM_BOOST_DATE_TIME],
...
if test "$ax_cv_boost_date_time" = yes; then
  AC_DEFINE(HAVE_BOOST_DATE_TIME,[1],[define if the Boost::DateTime library is available])
  ...
  for ax_lib in $ax_date_time_lib $ax_boost_date_time_lib boost_date_time; do
    AC_CHECK_LIB($ax_lib, main, [BOOST_DATE_TIME_LIB="-l$ax_lib" break])
  done
